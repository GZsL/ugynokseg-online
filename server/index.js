...
/**
 * ✅ AUTH MIDDLEWARE – FIX:
 * - a jó cookie nevet olvassa (ugynokseg_token)
 * - ugyanazt a JWT_SECRET-et használja (fallback-kal)
 */
...
// ✅ Default entry (NE legyen Cannot GET /)
app.get("/", (req, res) => {
...
function joinLobbyRoom({ roomCode, name, characterKey, password = null }) {
  ...
  const token = uid("tok");
  const pid = "p" + (r.players.length + 1);

  r.players.push({
    id: pid,
    name: String(name || "").trim() || `Játékos ${r.players.length + 1}`,
    characterKey,
    token,
    ready: false,
    isHost: false,
...
function startGame(roomCode) {
  const r = rooms.get(roomCode);
  if (!r) return { error: "Szoba nem található." };
  if (r.phase !== "LOBBY") return { error: "A játék már elindult." };

  const players = r.players || [];
  if (players.length < 2) return { error: "Legalább 2 játékos kell." };

  // mindenki ready?
  const allReady = players.every((p) => !!p.ready);
  if (!allReady) return { error: "Nem mindenki kész." };
...
// ✅ NEW: create lobby room (token-based) – csak belépett user
...
// NEW: join lobby room – vendég is jöhet
...
// NEW: send invite email(s) (host only token)
app.post("/api/send-invite", async (req, res) => {
  try {
    ...
    const hostPlayer = (r.players || []).find((p) => p && p.token === token && p.isHost);
    if (!hostPlayer) return res.status(403).json({ error: "Nincs jogosultság meghívót küldeni." });
    ...
    if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
      return res.status(500).json({ error: "SMTP nincs beállítva a szerveren." });
    }
    ...
    const text =
      `${hostPlayer.name} meghívót küldött az ÜGYNÖKSÉGHEZ!\n\n` +
...
function stateForPlayer(state, meIndex) {
  ...
  s._meIndex = meIndex;
  s._meId = (state.players || [])[meIndex] ? (state.players || [])[meIndex].id : null;
  return s;
}
